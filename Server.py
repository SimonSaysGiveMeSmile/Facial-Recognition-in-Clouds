...
def requestPicture(requestNumber, request, nameID, message, address):  # recursive function
    ...
    pictureSocket.bind(('', 5001))
    
    ipadd = str(pictureSocket.getsockname()[0])

    serverSocket.sendto(pickle.dumps([requestNumber, message, ipadd, 5001]), address)

    print("Picture request sent")

    pictureSocket.listen()
